...
from src.numerical_verification import a_violation_numcheck
...
def eval_MSE_sol(individual, num_consts, toolbox, true_data, ind2MSE, consts):
    ...
    try:
        ...
        V_dot_violations = np.count_nonzero(V_dot_vals >= -0.1 * (x_norm_squared))
        ...
        V_grad_mag = np.abs(np.mean(V_grad_mag) - 10)
        ...
        if MSE < 0.3:
            ...
            ind2MSE2 = DeapSimplifier(ind2MSE_str, scales=scales, should_print=False)
            a_violations = 0
            for i in range(1, 11):
                a_violations = a_violation_numcheck(ind2MSE2, 3500)
                if a_violations > 1:
                    MSE += a_violations
                    break
            MSE += a_violations
