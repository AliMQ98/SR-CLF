...
from src.ibex_artstein_optimizers import verify_clf_ibex_artstein
from src.codac_root_finder import find_codac_artstein_roots
...
IBEX_VERIFIER_ENABLED = False #True
IBEX_VERIFIER_TIMEOUT_SECONDS = 5.0  # per box; clean margins escalate to 2x, then 3x
IBEX_VERIFIER_VERBOSE = False #True

# --- IBEX exact Artstein verifier: maximize a with b approximately zero ---
# This is independent of the barrier verifier above.  It uses the exact
# symbolic a=grad(V).f and b=grad(V).G[:,1] expressions on 12 boxes.
IBEX_ARTSTEIN_ENABLED = False#True #False
IBEX_ARTSTEIN_TIMEOUT_SECONDS = 3*5.0  # per box
IBEX_ARTSTEIN_B_TOLERANCE = 1e-10
# The model enforces ||x||^2 >= radius^2; 1e-3 gives the requested 1e-6.
IBEX_ARTSTEIN_ORIGIN_RADIUS = 1e-3
IBEX_ARTSTEIN_VERBOSE = False #True

# --- CODAC root/feasibility paving: b = 0, a > 0 outside the origin ---
CODAC_ROOT_ENABLED = False
CODAC_ROOT_A_MIN = -1e-10
CODAC_ROOT_PAVING_EPS = 1e-3
CODAC_ROOT_VERBOSE = False #True

# --- numerical verifier (de/de1220/cmaes/shgo) ---
NUMERICAL_VERIFIER = None #"de"  # options: "shgo", "cmaes", "de", "de1220", None
...
MANIFOLD_EXACT_MARGIN_WEIGHT = 12000.0   # exact manifold: cap 1000 near margin 0.5
MANIFOLD_EXACT_MARGIN_PENALTY_MAX = 8000.0
# Near-zero sharpener for the EXACT check only (its margins are noise-free):
# concave root terms keep a non-vanishing slope at the finish line so tiny
# positive margins cannot be "parked on" cheaply, and the +SHIFT inside the
# 1/6-power term keeps a little pressure slightly BELOW zero, pushing
# candidates beyond the knife edge. SHIFT must stay safely below the true
# supremum magnitude of valid CLFs near the origin ball (LQR: |a| =
# 0.2504*r0^2 = 3.0e-7 at the ball edge), hence 1e-7 rather than 1e-6:
# at 1e-6 an accurately-polished LQR would be taxed ~24 while clean.
...
def _exact_near0_penalty(margin_max):
    """Concave finish-line terms for the exact check (noise-free margins):
    4*sqrt(m) + 64*m^(1/4) + 256*(m+SHIFT)^(1/6), each floored at 0.
    Non-vanishing slope at m=0 kills the 'park at a tiny positive margin'
    incentive; the SHIFT keeps pressure until margin <= -SHIFT (beyond 0)."""
    if not np.isfinite(margin_max):
        return 0.0
...
def _symbolic_a_b(ind2MSE, consts):
    """Build exact a=grad(V).f and scalar b=grad(V).G[:,1]."""
...
def _run_ibex_barrier_verification(ind2MSE, consts, timeout_seconds):
    ...
    a_expr, b_expr = _symbolic_a_b(ind2MSE, consts)
...
def _run_ibex_artstein_verification(ind2MSE, consts):
    """Maximize exact a subject to |b| <= IBEX_ARTSTEIN_B_TOLERANCE."""
    a_expr, b_expr = _symbolic_a_b(ind2MSE, consts)
    return verify_clf_ibex_artstein(
        a_expr,
        b_expr,
        SHGO_BOUNDS,
        origin_radius=IBEX_ARTSTEIN_ORIGIN_RADIUS,
        b_tolerance=IBEX_ARTSTEIN_B_TOLERANCE,
        timeout_seconds=IBEX_ARTSTEIN_TIMEOUT_SECONDS,
        verbose=IBEX_ARTSTEIN_VERBOSE,
    )


def _run_codac_root_verification(ind2MSE, consts):
    """Pave the exact bad set b=0, a>0 outside the origin ball."""
    a_expr, b_expr = _symbolic_a_b(ind2MSE, consts)
    return find_codac_artstein_roots(
        a_expr,
        b_expr,
        SHGO_BOUNDS,
        a_min=CODAC_ROOT_A_MIN,
        origin_radius=CLF_ORIGIN_EXCLUDE_RADIUS,
        paving_eps=CODAC_ROOT_PAVING_EPS,
        verbose=CODAC_ROOT_VERBOSE,
    )
...
def eval_MSE_breakdown(
    ...
    report = {
        ...
        "ibex_stage_margins": [],
        "ibex_artstein_ran": False,
        "ibex_artstein_penalty": 0.0,
        "ibex_artstein_a_max": np.nan,
        "ibex_artstein_a_upper_bound": np.nan,
        "ibex_artstein_completed_boxes": 0,
        "ibex_artstein_total_boxes": 0,
        "ibex_artstein_status": "not_run",
        "codac_ran": False,
        "codac_status": "not_run",
        "codac_inner_boxes": 0,
        "codac_boundary_boxes": 0,
        "codac_outer_boxes": 0,
        "codac_inner_box_bounds": np.empty((0, 4, 2)),
        "codac_boundary_box_bounds": np.empty((0, 4, 2)),
    ...
    try:
        ...
        # --- IBEX exact Artstein falsifier: max a on |b| <= tolerance ---
        if (
            verify_shgo
            and IBEX_ARTSTEIN_ENABLED
            and grid_mse < SHGO_MSE_GATE
        ):
            report["ibex_artstein_ran"] = True
            try:
                artstein_result = _run_ibex_artstein_verification(ind2MSE, consts)
                report["ibex_artstein_a_max"] = float(artstein_result.a_max)
                report["ibex_artstein_a_upper_bound"] = float(
                    artstein_result.a_upper_bound
                )
                report["ibex_artstein_completed_boxes"] = int(
                    artstein_result.n_complete
                )
                report["ibex_artstein_total_boxes"] = int(artstein_result.n_boxes)
                report["ibex_artstein_status"] = artstein_result.status
                # As with the barrier verifier, only a feasible positive
                # maximum is a counterexample and receives a margin penalty.
                if np.isfinite(artstein_result.a_max):
                    report["ibex_artstein_penalty"] = (
                        20 * SHGO_MARGIN_WEIGHT * max(artstein_result.a_max, 0.0)
                    )
            except Exception as exc:
                report["ibex_artstein_status"] = (
                    f"error: {type(exc).__name__}: {exc}"
                )
        elif verify_shgo and IBEX_ARTSTEIN_ENABLED:
            report["ibex_artstein_status"] = "skipped_by_mse_gate"
        elif not IBEX_ARTSTEIN_ENABLED:
            report["ibex_artstein_status"] = "disabled"

        # --- CODAC paving of the exact Artstein bad set ---
        if verify_shgo and CODAC_ROOT_ENABLED and grid_mse < SHGO_MSE_GATE:
            report["codac_ran"] = True
            codac_result = _run_codac_root_verification(ind2MSE, consts)
            report["codac_status"] = codac_result.status
            report["codac_inner_boxes"] = int(codac_result.n_inner_boxes)
            report["codac_boundary_boxes"] = int(codac_result.n_boundary_boxes)
            report["codac_outer_boxes"] = int(codac_result.n_outer_boxes)
            report["codac_inner_box_bounds"] = codac_result.inner_boxes
            report["codac_boundary_box_bounds"] = codac_result.boundary_boxes
        elif verify_shgo and CODAC_ROOT_ENABLED:
            report["codac_status"] = "skipped_by_mse_gate"
        elif not CODAC_ROOT_ENABLED:
            report["codac_status"] = "disabled"
        ...
        if (
            ...
            if mex is not None and mex.status == "ok":
                ...
                # finish-line sharpener: fires whenever margin > -SHIFT,
                # including the zero-violation knife-edge case, so candidates
                # are pushed BEYOND 0 instead of parking on it
                if mex.n_roots > 0:
                    report["manifold_exact_near0"] = _exact_near0_penalty(
                        mex.margin_max
                    )
                    report["manifold_exact_penalty"] = float(
                        report["manifold_exact_penalty"]
                        + report["manifold_exact_near0"]
                    )
                report["final_mse"] = float(
                    report["final_mse"] + report["manifold_exact_penalty"]
                )
        elif MANIFOLD_EXACT_CHECK_ENABLED:
            report["final_mse"] += float(MANIFOLD_EXACT_MSE_GATE) * 1.3
