...
def eval_MSE_sol(individual, num_consts, toolbox, true_data, ind2MSE, consts):
    ...
    try:
        ...
        ind2MSE_str = substitute_paramsCoef(str(ind2MSE), consts)
        ind2MSE_gr = DeapSimplifier(ind2MSE_str, should_print=False)
        ...
        dV_dx2_at_x2_0 = (diff(ind2MSE_gr, x2).subs({x2: 0}))
        if x1 not in dV_dx2_at_x2_0.free_symbols:
            MSE += 1e6

        # 2) substitute x1 = 0 in dV/dx1, and penalize if it has no x2
        dV_dx1_at_x1_0 = (diff(ind2MSE_gr, x1).subs({x1: 0}))
        if x2 not in dV_dx1_at_x1_0.free_symbols:
            MSE += 1e6

        d2 = dV_dx2_at_x2_0.subs({x1: 1})
        d1 = dV_dx1_at_x1_0.subs({x2: 1})
        d2 = float(d2.evalf())
        d1 = float(d1.evalf())
        
        # penalize if both gradients are negative
        if d1 < 0 and d2 < 0:
            MSE += 1e3
        ...
        V_dot_violations = np.count_nonzero(V_dot_vals >= -0.25 * x_norm_squared)
        ...
        V_grad_mag = np.abs(np.mean(V_grad_mag) - 1)
