...
def eval_MSE_sol(individual, num_consts, toolbox, true_data, ind2MSE, consts):
    ...
    try:
        ...
        try:
            V_vals = individual(X1, X2, consts)
        ...
        V_dot_violations = np.count_nonzero(V_dot_vals >= -0.01 * x_norm_squared)
        ...
        V_grad_mag = np.abs(np.mean(V_grad_mag) - 6.183163)
